...
export default function DiffChecker() {
  ...
  const changes = useMemo(
    () =>
      hasBothInputs
        ...
        : [],
    [debouncedOriginal, debouncedUpdated, hasBothInputs, ignoreWhitespace, ignoreCase]
  );
  ...
  return (
    <ToolLayout>
      <OutputPanel>
        <div className="panel-label">Original Source</div>
        <textarea
          className="textarea"
          placeholder="Paste the original code or text…"
          value={originalText}
          onChange={(event) => setOriginalText(event.target.value)}
          style={{ minHeight: 160, marginBottom: 8, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace' }}
        />
        {originalText ? (
        ...
        <div className="panel-label">Updated Source</div>
        <textarea
          className="textarea"
          placeholder="Paste the revised code or text…"
          value={updatedText}
        ...
        {!hasBothInputs ? (
          <EmptyState
            iconName="Layers"
            title="Compare two versions line by line"
            message="Paste the original source and the updated source to inspect additions, deletions, and unchanged blocks."
          />
        ) : (
          <>
            <MetricGrid
              items={[
                {
                  ...
                  description: 'New lines in the updated version',
                  tone: stats.added ? 'success' : 'default',
                },
                {
                  label: 'Removed',
                  value: String(stats.removed),
                  description: 'Lines missing from the updated version',
                  tone: stats.removed ? 'error' : 'default',
                },
                {
                  label: 'Unchanged',
                  value: String(stats.same),
                  description: 'Lines that stayed the same',
                },
              ]}
              marginBottom={16}
            />
            ...
            {viewMode === 'unified' ? (
              ...
              >
                {rows.map((row, index) => {
                  const leftPalette = getToneStyles(row.leftTone);
                  const rightPalette = getToneStyles(row.rightTone);
                  ...
                  return (
                    ...
                    >
                      <div
                        style={{
                          background: leftPalette.background,
                          border: `1px solid ${leftPalette.border}`,
                          borderRadius: 'var(--radius-md)',
                          padding: '10px 12px',
                          minHeight: 40,
                          fontSize: 12,
                          lineHeight: 1.7,
                          color: leftPalette.color,
                          whiteSpace: 'pre-wrap',
                          wordBreak: 'break-word',
                          fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace',
                        }}
                      ...
                      <div
                        style={{
                          background: rightPalette.background,
                          border: `1px solid ${rightPalette.border}`,
                          borderRadius: 'var(--radius-md)',
                          padding: '10px 12px',
                          minHeight: 40,
                          fontSize: 12,
                          lineHeight: 1.7,
                          color: rightPalette.color,
                          whiteSpace: 'pre-wrap',
                          wordBreak: 'break-word',
                          fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace',
                        }}
      ...
      <OptionsPanel>
        ...
        <button
          ...
          onClick={() => downloadText(diffReport, 'diff-report.txt', 'text/plain;charset=utf-8')}
          disabled={!diffReport}
        >
        ...
        <div className="privacy-note">Diff analysis runs client-side with the `diff` package</div>
      </OptionsPanel>
    </ToolLayout>
  );
}
